...
func TestTransformation_ParseExecute(t *testing.T) {
	tests := []struct {
		...
		{
		    name:             "power function works with integers",
		    recipe:           "1 <- power(\"2\", 1)",
		    input:            "3\n2\n",
		    want:             "8.000000\n4.000000\n",
		},
		{
            name:             "power function works with floats",
            recipe:           "1 <- power(\"2.5\", 1)",
            input:            "3\n2\n",
            want:             "15.625000\n6.250000\n",
        },
        {
            name:             "power function returns error for non-numeric number parameter",
            recipe:           "1 <- power(1, \"2\")",
            input:            "3\napple\n",
            wantErr:          true,
            wantErrText:      "line 2 / column 1: power(): unrecognized number 'apple' for num parameter",
        },
         {
            name:             "power function returns error for non-numeric power parameter",
            recipe:           "1 <- power(\"2\", 1)",
            input:            "3\napple\n",
            wantErr:          true,
